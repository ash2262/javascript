...
(function(w) {
	...
	impl = {
		...
		initFromCookie: function() {
			...
			if (subcookies.s && (subcookies.r || subcookies.nu)) {
				this.r = subcookies.r;
				//  urlHash = BOOMR.utils.MD5(d.URL);  Change Date: 6th Sep 2020
				    urlHash = BOOMR.utils.SHA1(d.URL);
			    //	docReferrerHash = BOOMR.utils.MD5((d && d.referrer) || ""); Change Date: 6th Sep 2020 
					docReferrerHash = BOOMR.utils.SHA1((d && d.referrer) || "");
		...
		page_unload: function(edata) {
			...
			this.updateCookie(
				(!impl.navigationStart && impl.strict_referrer) ? {
					// "r": BOOMR.utils.MD5(d.URL)     Change Date: 6th Sep 2020 
					"r": BOOMR.utils.SHA1(d.URL)
				} : null,
		...
		_iterable_click: function(name, element, etarget, value_cb) {
			...
			if (etarget && etarget.nodeName && etarget.nodeName.toUpperCase() === element) {
				...
				this.updateCookie(
					{
						// "nu": BOOMR.utils.MD5(value) Change Date: 6th Sep 2020 
						"nu": BOOMR.utils.SHA1(value)
					},
					"cl");
	...
	BOOMR.plugins.RT = {
		 ...
		 */
			...
			//WBG - Set metricType=boomerang, to differentiate events
			BOOMR.addVar("metricType", "boomerang");
			impl.addedVars.push("metricType");
